...
| `polls` | 투표 — `kind`(`date`/`yesno`/`choice`), `options[]`, `votes{이름:[옵션id]}`, `closed`, `decidedOptionId` |
...
| | 고를 수 있는 개수 | 이름 공개 | 마감하면 |
|---|---|---|---|
| 날짜 정하기 | 여러 개 | 보임 | 정한 날이 달력에 등록됨 |
| 찬반 묻기 | 하나 | **안 보임** | 표 많은 쪽으로 결론 |
| 후보 중 고르기 | 하나 | 보임 | 표 많은 후보가 당선 |
